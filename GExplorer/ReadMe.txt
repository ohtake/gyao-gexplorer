...
再放送が始まった時に古いキャッシュが残ったままだと表示内容が古いままのことがあります
古いキャッシュを自動的に消すことはないので，定期的に以下のメニューを実行してください
・ツール > キャッシュの削除 > クロール結果で到達不可能なキャッシュの削除
・ツール > 配信期限辞書の整理 > クロール結果で到達不可能なエントリーの削除

◆既知の不具合 (解決方法不明)
プレーヤフォームを閉じた後に動作が不安定になりがち (再生を停止させてからウィンドウを閉じるとよい)
マルチモニタ環境での動作 (確認する環境がないからどうしようもないような)
[画面のプロパティ] [スクリーンセーバー] [パスワードによる保護] が有効だとスクリーンセーバの抑止が出来ないことがある
WMPダブルクリックで全画面に行かないことがある (右クリックから一旦全画面にすればその後は解決)
...
外部コマンドで置換後の文字列に { や } が入っていたら動かなくなる
