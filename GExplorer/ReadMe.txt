パソコンテレビ GyaO 専用ブラウザ GExplorer 2.0.5.0
...
◆ユーザID
パソコンテレビGyaOを利用する際にはユーザIDが必要となるため
GExplorer は起動時にユーザIDを読みにいきます．
起動前に IE でユーザ登録を済ませておいてください．
以下の順序で読み取りを試みます．
・ActiveX の GyaOTool が利用しているレジストリを読む
・ファイルシステムからクッキーを読む
読み取りに失敗した場合には手動で ID を設定する必要があります．

◆操作方法
◇ジャンルタブ (メイン画面の上部)
パソコンテレビGyaOのジャンルの一覧です．
読み込んでいないジャンルのタブを選択するとクロールを実行し
結果をクロール結果ビューに表示します．
タブをダブルクリックすると読み込みなおします．
タブを右クリックするこもできます．
...
◇ウェブブラウザ
パソコンテレビGyaOの閲覧に特化したIEベースのウェブブラウザを内蔵しています．
...
・高速化の対象となるNGコンテンツ
　　主語が ContentId または Title であり尚且つ述語が Equals の
