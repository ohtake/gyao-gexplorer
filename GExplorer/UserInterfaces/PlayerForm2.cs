...
namespace Yusen.GExplorer.UserInterfaces {
	public sealed partial class PlayerForm2 : BaseForm, INotifyPropertyChanged, IPlayerFormBindingContract {
		...
		private void OpenVideo() {
			...
			//年齢チェック
			Match matchAge = PlayerForm2.regexAgeThreshold.Match(body);
			if (matchAge.Success) {
			...
			//asx.phpを検索
			Match matchAsxPhp = PlayerForm2.regexAsxPhp.Match(body);
			...
			// asx をダウンロード
			string tempFile = Path.Combine(Program.GetWorkingDirectory(WorkingDirectory.Temp) , PlayerForm2.tempAsxFilename);
			Application.DoEvents();
			...
			if (!string.IsNullOrEmpty(this.options.StreamingServerAltAddress)) {
				...
				using (StreamWriter writer = new StreamWriter(tempFile)) {
					asxText = asxText.Replace(PlayerForm2.StreamingServerName, altAddr);
			...
			Match matchBannerKeyValue = PlayerForm2.regexBannerKeyValue.Match(body);
		...
		private void HideUi() {
			...
			loc += PlayerForm2.WmpMarginSize;
			//WMPのビデオ部分のサイズ計算
			Size size = this.splitContainer2.ClientSize;
			size -= PlayerForm2.WmpUiSize;
			//リージョン設定
		...
		private Uri CreateBannerUri(string dartTag) {
			long ord = (long)(this.rand.NextDouble() * PlayerForm2.OrdMax);
			return new Uri(
		...
		private bool IsCm {
			get {
				string entryUrl = this.currentAttribs[PlayerForm2.AttribNameEntryUrl];
		...
		private void PlayerForm_Load(object sender, EventArgs e) {
			...
			BindingContractUtility.BindAllProperties<PlayerForm2, IPlayerFormBindingContract>(this, this.options);
		...
		private void tsmiResizeToVideoResolution_Click(object sender, EventArgs e) {
			...
			for(int i=0; i<2; i++) {
				...
				//2回リサイズを試みる
				Size delta = distSize + PlayerForm2.WmpUiSize - this.wmpMain.Size;
				this.Size += delta;
			}
		}
		...
		private void wmpMain_OpenStateChange(object sender, _WMPOCXEvents_OpenStateChangeEvent e) {
			switch ((WMPOpenState)e.newState) {
				case WMPOpenState.wmposMediaOpen:
					...
					//バナー表示
					Match dartTagMatch = PlayerForm2.regexDartTag.Match(entryUrl);
				...
				case WMPPlayState.wmppsMediaEnded:
					if (this.IsLastEntry) {
						if (this.currentChapter.HasValue) {//チャプタモード
							if (PlayerForm2.LastCallTitle.Equals(this.wmpMain.currentMedia.name)) {
								goto endOfCont;
							} else {
								//次のチャプタ
								this.currentChapter++;
		...
		private void UpdateStatusbarText() {
			if (null != this.currentContent) {
				...
				if (this.currentAttribs.TryGetValue(PlayerForm2.AttribNameEntryUrl, out entryUrl)) {
					matchClipNo = PlayerForm2.regexClipNo.Match(entryUrl);
					matchClipBegin = PlayerForm2.regexClipBegin.Match(entryUrl);
				}
